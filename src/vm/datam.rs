                ...
                if schar == '!' && echar == ')' {
                    ...
                    match xop.as_str() {
                        "!byteele" | "!be" | "!arrayele" | "!ae" => {
                            ...
                            // * indexing of indexing using XOping syntax
                            let tindex;
                            (tdata, tindex) = sdata.split_once(',').expect(&format!("ERRR:{}:DataM:Compile:XOp:{}:Extracting args:{}", smsg, xop, sdata));
                            sarg1 = tdata.as_str();
                    ...
                    let dm = DataM::compile(ctxt, sarg1, stype, &format!("{}:XOp-{}:{}",smsg, xop, sarg1));
    ...
    pub fn get_isize(&self, ctxt: &mut Context) -> Result<isize, String> {
        match self {
            ...
            Self::XOp(xdata) => {
                ...
                if ival.is_err() {
                    return Err(format!("DataM:GetISize:XOp:{:?}:{}", xdata, ival.unwrap_err()));
                }
    ...
    pub fn get_string(&self, ctxt: &mut Context) -> Result<String, String> {
        match self {
            ...
            Self::XOp(xdata) => {
                ...
                if sdata.is_err() {
                    return Err(format!("DataM:GetString:XOp:{:?}:{}", self, sdata.unwrap_err()));
                }
    ...
    pub fn get_bufvu8(&self, ctxt: &mut Context) -> Result<Vec<u8>, String> {
        match self {
            ...
            Self::XOp(xdata) => {
                ...
                if bval.is_err() {
                    return Err(format!("DataM:GetBuf:XOp:{}", bval.unwrap_err()));
                }
    ...
    */
    ...
    pub fn get_arrayelement(&self, ctxt: &mut Context, index: usize) -> Result<Variant, String> {
        match self {
            ...
            Self::XOp(xdata) => {
                ...
                if vval.is_err() {
                    return Err(format!("DataM:GetArrayEle:XOp:{:?}:{}", self, vval.unwrap_err()));
                }
                return vval;
            }
        }
    ...
    fn get_value(&self, ctxt: &mut Context) -> Result<Variant, String> {
        match self {
            ...
            Self::XOp(xdata) => {
                ...
                if vval.is_err() {
                    return Err(format!("DataM:GetVal:XOp:{}", vval.unwrap_err()));
                }
    ...
    pub fn get_type_value(&self, ctxt: &mut Context) -> Result<(VDataType, Variant), String> {
        match self {
            ...
            Self::XOp(xdata) => {
                ...
                if oval.is_err() {
                    return Err(format!("DataM:GetTypeVal:XOp:{}", oval.unwrap_err()));
                }
                let oval = oval.unwrap();
    ...
    fn get_bufvu8_mut<'a>(&'a self, ctxt: &'a mut Context, smsg: &str) -> &'a mut Vec<u8> {
        match self {
            ...
            Self::XOp(_xdata) => {
                panic!("ERRR:{}:DataM:GetBufVu8Mut:XOp:{:?}:Not supported", smsg, self);
            }
        }
    }
    ...
    pub fn set_isize(&self, ctxt: &mut Context, vvalue: isize) -> Result<(), String> {
        match  self {
            ...
            Self::XOp(_xdata) => {
                return Err(format!("DataM:SetISize:XOp:{:?}:Not supported", self));
            }
        }
    }
    ...
    fn set_string(&self, ctxt: &mut Context, vvalue: String) -> Result<(), String> {
        match  self {
            ...
            Self::XOp(_xdata) => {
                return Err(format!("DataM:SetString:XOp:{:?}:Not supported", self));
            }
        }
    }
    ...
    pub fn set_bufvu8(&self, ctxt: &mut Context, vvalue: Vec<u8>) -> Result<(), String> {
        match  self {
            ...
            Self::XOp(_xdata) => {
                return Err(format!("DataM:SetBuf:XOp:{:?}:Not supported", self));
            }
        }
    }
    ...
    pub fn set_value(&self, ctxt: &mut Context, vvalue: Variant, bforcelocal: bool) -> Result<(), String> {
        match  self {
            ...
            Self::XOp(_xdata) => {
                return Err(format!("DataM:SetValue:XOp:{:?}:Not supported", self));
            }
        }
    }
